...
# ⚙️ 1. Config
# ==========================================
st.set_page_config(page_title="Nami Admin V109", layout="wide", page_icon="🧾")

ADMIN_PASSWORD = "3457"
APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxlUwV9CaVXHBVmbvRwNCGaNanEsQyOlG8f0kc3BHAS_0X8pLp4KxZCtz_EojYBCvWl6w/exec" # 🟢 ใส่ URL เดิม
...
def load_data():
    ...
    # 1. Config (อ่านแบบระบุเซลล์ เพื่อความชัวร์)
    ws_c = sh.worksheet("Config")
    # สมมติโครงสร้าง: A1=Key, B1=Value -> A2=ShopName, B2=ค่า
    # อ่าน B2, B3, B4 โดยตรง
...
def save_shop_config(sh, name, tax, addr):
    try:
        ...
        # บันทึกทับเซลล์ตรงๆ (Hardcode cell) เพื่อแก้ปัญหาหาไม่เจอ
...
def generate_pdf_precise(data, items, doc_type, run_no, date_str, is_vat, logo_data):
    ...
    def draw_section(y_start, is_copy):
        # y_start คือจุดบนสุดของครึ่งกระดาษ (297 หรือ 148.5)
        ...
        # 2. Shop Box (Top Right) - Fixed Position
        ...
        c.setFont(FONT_NAME, 12) # Shop Name
        c.drawString(box_x + 3*mm, box_y + box_h - 6*mm, data['s_n'])
        
        c.setFont(FONT_NAME, 10) # Address
        # Wrap Address manually
        addr = data['s_a']
        addr_lines = [addr[i:i+60] for i in range(0, len(addr), 60)] # Simple wrap
        ay = box_y + box_h - 11*mm
        for l in addr_lines[:3]:
            c.drawString(box_x + 3*mm, ay, l)
            ay -= 4*mm
        
        # 3. Header Title (Center)
        ...
        c.rect(15*mm, rect_btm, width - 30*mm, rect_h) # Outer Rect
        c.line(width/2 + 20*mm, rect_top, width/2 + 20*mm, rect_btm) # Vertical Split
        
        # Left Info (Customer)
        lx = 18*mm; ly = rect_top - 5*mm
        c.drawString(lx, ly, f"ชื่อลูกค้า : {data['c_n']}")
        c.drawString(lx, ly - 6*mm, f"ที่อยู่ : {data['c_a'][:60]}") # Line 1
        c.drawString(lx, ly - 10*mm, f"       {data['c_a'][60:110]}") # Line 2
        ...
        c.rect(15*mm, tbl_header_y - 7*mm, width - 30*mm, 7*mm, fill=1, stroke=1) # Header BG
        c.setFillColorRGB(0,0,0)
        
        # Grid Cols: No, Item, Qty, Price, Total
        ...
        # Draw Vertical Lines (Full Height of Table)
        table_btm = rect_btm - 90*mm
        c.rect(15*mm, table_btm, width-30*mm, 90*mm) # Main Box
        for x in xs[1:-1]:
        ...
        sum_y = table_btm
        c.line(xs[4], sum_y, xs[4], sum_y - 35*mm) # Vertical line for summary
        c.line(xs[5], sum_y, xs[5], sum_y - 35*mm)
        c.rect(xs[4], sum_y - 35*mm, xs[5]-xs[4], 35*mm) # Right box
        c.rect(15*mm, sum_y - 35*mm, width-30*mm, 35*mm) # Full Footer box
        ...
        for l, v in zip(lbls, vals):
            ...
            c.line(xs[4], curr_sy-1*mm, xs[5], curr_sy-1*mm) # underline
            curr_sy -= 7*mm
...
# Main UI
st.title("🧾 ระบบออกใบกำกับภาษี (V109 Precision)")

tab1, tab2 = st.tabs(["📝 ออกบิล / ข้อมูลร้าน", "☁️ รายการคิว"])

with tab1:
    # 1. Shop Info (Editable)
    with st.expander("🏠 แก้ไขข้อมูลร้าน / โลโก้"):
        c1, c2 = st.columns(2)
        new_sn = c1.text_input("ชื่อร้าน", value=s_n)
        new_st = c2.text_input("Tax ID", value=s_t)
        new_sa = st.text_area("ที่อยู่", value=s_a)
    ...
    if st.session_state.cart:
        ...
        if st.button("🖨️ ยืนยันการออกบิล", type="primary", use_container_width=True):
            ...
            with st.spinner("Processing..."):
                ...
                # 1. Update Sheet (SalesLog & Config)
                try:
                    # Next Run No
                ...
                except: st.warning("Sheet update error (but PDF will generate)")
